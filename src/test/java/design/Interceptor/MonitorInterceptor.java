...
public class MonitorInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        try {
            ...
            //invocation.proceed() 将执行权交给下一个拦截器;
...
//                            logger.warn("sqlId={}执行耗时{}毫秒，超过阀值{}，执行的sql语句是[{}], 参数值：[{}]", sqlId, usedTimeInMs, slowSqlTimeout, sql.getSql(), parameterObject);
//                        }
//
//                        // 模拟报警
...
//                }
                logger.warn("sqlId={}执行耗时{}毫秒，超过阀值{}，执行的sql语句是[{}], 参数值：[{}]", sqlId, usedTimeInMs, slowSqlTimeout, sql.getSql(), parameterObject);
                //System.err.println(String.format("sqlId=%s执行耗时%s毫秒，超过阀值%s，执行的sql语句是[%s], 参数值：[%s]", sqlId, usedTimeInMs, slowSqlTimeout, sql.getSql(), parameterObject));
